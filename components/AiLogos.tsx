// Simplified SVG logo representations for major AI platforms
...
export function GeminiLogo({ className = "w-8 h-8" }: { className?: string }) {
  return (
...
export function PerplexityLogo({ className = "w-8 h-8" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-label="Perplexity">
...
export function CopilotLogo({ className = "w-8 h-8" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-label="Microsoft Copilot">
...
const aiPlatforms: AiLogoProps[] = [
  { name: "ChatGPT", Icon: OpenAILogo, color: "#10a37f" },
  { name: "Gemini", Icon: GeminiLogo, color: "#4285f4" },
  { name: "Claude", Icon: ClaudeLogo, color: "#d97706" },
  { name: "Perplexity", Icon: PerplexityLogo, color: "#6366f1" },
...
export default function AiLogos() {
  return (
    <div className="flex flex-wrap items-center justify-center gap-10 md:gap-16">
      {aiPlatforms.map(({ name, Icon, color }) => (
        <div key={name} className="flex flex-col items-center gap-2 opacity-50 hover:opacity-80 transition-opacity">
          <Icon className="w-7 h-7 md:w-8 md:h-8" style={{ color }} />
          <span className="text-xs font-medium text-muted tracking-wide">{name}</span>
        </div>
      ))}
    </div>
  );
